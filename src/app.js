...
function makePre(){
  ...
  for(const tr of ['d1','d2']){
    ...
    const opp={};
    for(const t of tierTeams(tr)) opp[t.id]=[];
    for(const g of games){
      push(g.h.id,{o:g.v.id,home:true,t:g.type,wk:g.week});
      push(g.v.id,{o:g.h.id,home:false,t:g.type,wk:g.week});
      opp[g.h.id].push(g.v.rating); opp[g.v.id].push(g.h.rating);
    }
    // Strength of schedule before a ball is kicked: nobody has a result to go
    // on, so it is what it always is — the average rating of the slate.
    const list=tierTeams(tr);
    for(const t of list){
      const o=opp[t.id];
      sn.teams[t.id].sos = Math.round(o.length ? o.reduce((a,b)=>a+b,0)/o.length : 50);
    }
    list.slice().sort((a,b)=>sn.teams[b.id].sos-sn.teams[a.id].sos)
...
function schedHTML(id){
  ...
  let head = pre
    ? esc(me.name)+' opens '+sn.season+' in '+tierOf(me)+', ranked #'+R[id]+' by rating. '+
      ...
      'rank #'+avgRk+' and rating '+avgRt+', the '+ord(me.sosRank)+' hardest slate in the tier.'+
      rivalNote
  ...
  for(const tr of tiers){
    ...
    for(const c of CONFS) for(const d of DIVS.filter(x=>x.indexOf(c+' ')===0)){
      ...
      tb.innerHTML='<thead><tr><th class="n">Rk</th><th>Team</th><th class="n">Rtg</th>'+
        (pre?'<th class="hidesm">Home</th>':'<th class="n">Div</th><th class="n">W-L</th>')+
        '<th class="n">SOS</th>'+
        '</tr></thead>';
      const body=el('tbody');
...
function renderFlat(sn,pre,tiers,get,sortBy,host){
  ...
  for(const tr of tiers){
    ...
    tb.innerHTML='<thead><tr><th class="n">Rk</th><th>Team</th><th class="hidesm">Division</th>'+
      '<th class="n">Rtg</th>'+(pre?'':'<th class="n">W-L</th>')+
      '<th class="n">SOS</th></tr></thead>';
    const body=el('tbody');
    for(const id of order){
      const t=get(id), row=el('tr');
      if(!pre){
