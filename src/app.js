...
function renderMapFocus(sn){
  const ix=byId(), me=ix[mapFocus], games=sn.sched[mapFocus];
  ...
  $('#mapHost').querySelectorAll('.dot').forEach(el2=>{
    const gi=+el2.dataset.g, t=ix[el2.dataset.id];
    const showTip=e=>{
...
function renderMap(){
  ...
  const show=S.teams.filter(t=>(tier==='all'||t.tier===tier)&&mapFilter.has(groupOf(t)));
